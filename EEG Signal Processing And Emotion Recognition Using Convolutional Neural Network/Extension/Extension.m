...
for iiff = 1:length(ifq)
    if ifq(iiff) >= 8 && ifq(iiff) < 12
        alpha(iiffa) = ifq(iiff);
        iiffa = iiffa+1;
    elseif ifq(iiff) >= 12 && ifq(iiff) < 30
        beta(iiffb) = ifq(iiff);
        iiffb = iiffb+1;
    elseif ifq(iiff) >= 4 && ifq(iiff) < 7
        btheta(iiffc) = ifq(iiff);
        iiffc = iiffc+1;
    end
    if iiff == length(ifq)
        if length(beta) == 1
...
% end
load('Labelsdeap.mat')
load('labelscombo.mat')
load('labelscomboval.mat')

if inid <= 15
    predf = Labelsdeap{inid+1}
    predw = Labelsdeap{inid+5}
    predv = labelscomboval{inid+1}
    predc = labelscombo{inid+5}
else
    predf = Labelsdeap{inid}
    predw = Labelsdeap{inid}
    predv = labelscomboval{inid}
    predc = labelscombo{inid}
end

load('out.mat')
load('outw.mat')

%accuracy for initial 3sec
accuracy = accu(out,output,1);
accuracyw = accu(outw,outputw,2);
accuracyv = accu(out,output,1);
accuracyvw = accu(outw,outputw,2);

%accuracy for last 3sec
accuracyl = accu(out,output,3);
accuracywl = accu(outw,outputw,4);
accuracyvl = accu(out,output,3);
accuracyvwl = accu(outw,outputw,4);

%filtered signals alpha,beta and theta
accuracya = accu(x,alpha,7);
accuracyb = accu(x,beta,8);
accuracyc = accu(x,btheta,9);

%unfiltered signal or raw eeg signal
accuracyu = accu(x,xf,5);