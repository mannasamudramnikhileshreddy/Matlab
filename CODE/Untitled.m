...
for i=1:1:n
    ...
    S(i).type='N';
    if i >= 0 && i <= 15
        plot(S(i).xd,S(i).yd,'ro','linewidth',0.5);
    elseif i > 15 && i <= 30
        plot(S(i).xd,S(i).yd,'go','linewidth',0.5);
    elseif i > 30 && i <= 50
        plot(S(i).xd,S(i).yd,'bo','linewidth',0.5);
    end
    hold on;
end
...
plot(S(n+1).xd,S(n+1).yd,'k^','linewidth',2);
...
S1(n+1).yd=sink.y;
plot(S1(n+1).xd,S1(n+1).yd,'k^','linewidth',2);
% % ETX=50*0.000000001;
% % ERX=50*0.0000000001;
ERX=50*0.0000000005;
 Efs=10*0.0000000000001;     %amplification energy when d is less than d0
