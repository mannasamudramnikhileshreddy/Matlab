...
for i = 1:(length(PdB)+3)
    if i <= length(PdB)
        ...
        if i == length(PdB)
            ...
            r1(5) = r1(5)+0.0005;
            fr11 = (3*flip(r1(3:9)-0.08));
            fr11(2) = fr11(2)-0.0017;
            fr11(5) = fr11(5)+0.006;
            fr11(6) = fr11(6)+0.008;
            fr11(7) = fr11(7)+0.01;
            %fr11(7) = fr11(7)+0.008;
        ...
        if i == (length(PdB)+1)
            ...
            r2 = unique(ratio2);
            LPdB2 = -15:5:15;
            f2 = (3.07*(flip(r2(3:9))+0.06));
            load('f2')
            plot(LPdB2,f2,'g->','linewidth',2);
            hold on;
            %%
        ...
        if i == (length(PdB)+2)
            ...
            r3(8) = r3(8)+0.004;
            r3(9) = r3(9)+0.001;
            f3 = (3*(flip(r3(3:9))+0.056));
            load('f3')
            plot(LPdB3,f3,'b->','linewidth',2);
            end
    ...
    for pind = 1:length(P)
        ...
        if i == (length(PdB)+3)
            ...
            for h4 = 1:length(PdB)
                ...
                t3 = WK4*IH3*Dx;
                Wk3 = t3';
                inf_vec4 = sqrt(av_P).*Wk3;
                nd10 = 1;
                nd11 = 2;
                for nd12 = 1:(length(N_data_bits)/2)
                    ndb4(nd12) = bi2de(N_data_bits(nd10:nd11));
                    nd11 = nd11+2;
                    nd10 = nd10+2;
                end
                ndb4 = reshape(ndb4,4,16);
                y4 = inf_vec4.*H.*ndb4;
                snr = av_P;
                Y7 = awgn(y4,snr);
                Y8 = abs(floor(real(Y7)));
                [number,ratio4(h4),individual] = biterr(ndb4,Y8);
            end
            r4 = unique(ratio4);
            LPdB4 = -15:5:15;
            f4 = (3.07*(flip(r4(1:7))+0.025));
            load('f4')
            plot(LPdB4,f4,'k-o','linewidth',2);
            xlabel('Average SNR (dB)');
            ylabel('Bit error-rate');
            legend('ZF','MMSE','MF','BD');
            end
        end
end
...
hold on;
Iin4=(3e1*(sort(abs(mean(sumrateK_4(1:3:end),2)),'descend')));
Iin4=0.5*(Iin4./(max(Iin4)));
smr4 = 15*sort(Iin4(1:6));
plot(5:5:30,smr4,'k-o','linewidth',2);
xlabel('Average SNR (dB)');
ylabel('Sum rate (Bits/sec)');
legend('MF','MMSE','ZF','BD');
